...
int main(int argc, char* argv[]){
   ...
   while( fgets(line, MAX_STRING_LENGTH, in) != NULL ){
      extract_alpha_num(line, alpha_num);// extracts alphanum char ... we want to extrac a,d,p,w char
      fprintf(out, "%s\n", alpha_num);
   }
...
void extract_alpha_num(char* s, char* a){
   ...
   while(s[i]!='\0' && i<MAX_STRING_LENGTH){
      if( isdigit( (int)s[i]) ) a[j++] = s[i]; // if s[i] is alphanum , put it into array a :)
      i++;
   }
   a[j] = '\0';
}
